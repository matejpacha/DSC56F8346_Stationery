...
#      program code --> pROM
#         constants --> xROM
#      dynamic data --> xROM copied to internal xRAM
#         variables --> int xRAM
#
...
SECTIONS 
{
    ...
    .p_flash_ROM_data :
    {
        # initialized __pmem constants in p_flash ROM
        
        * (.const.data.char.pmem)
        * (.const.data.pmem)
        
        # save address for pROM data we will copy to pRAM
        _datap_ROM_addr =.; 
        
    } >> .p_flash_ROM

    .pmemdata_image :  AT(_datap_ROM_addr)  # data load address in pFlash
    {
    ...
    .x_flash_ROM_data :
    {
        ...
        * (.const.data)
        ...
        # remember address for ROM data we will copy to xRAM
        _data_ROM_addr =.; 
              
    } > .x_flash_ROM

    .data_image : AT(_data_ROM_addr) # data load address in xFlash
    {
        # the data sections (init values saved in xROM)
        ...
        * (.data)
        ...
        # calculate address for next part of xROM data
...
#   } >> .x_external_RAM    # external RAM for data
    ...
    .fardata_image : AT(_data2_ROM_addr) # data load address in xFlash (after the first part)
    {
        # far data sections (init values saved in xROM)
    ...
    } > .x_external2_RAM   # far external RAM for fardata
