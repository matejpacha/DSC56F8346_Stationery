            ...
            void (*destructor) ( unsigned int *thisp ) )
{
	...
	if(padding_size)
	{
		*(unsigned char *)(ptr+padding_size-sizeof(_CXXABI_SIZE_T)) = element_count;
	}
	...
	if(constructor)
	{
		...
		{
			constructor((unsigned int *)arrayptr);
            ...
            void (*destructor) ( unsigned int *thisp ) )
{
	...
	if(padding_size)
	{
		*(unsigned char *)((unsigned char *)ptr+padding_size-sizeof(_CXXABI_SIZE_T)) = element_count;
	}
	...
	if(constructor)
	{
		...
		arrayptr=(unsigned char *)ptr+padding_size;
		if(destructor)
		{
			for(; element_count>0; --element_count)
			{
	...
	return (unsigned char *)ptr+padding_size;
}
            ...
            void (*destructor) ( unsigned int *thisp ) )
{
	if(array_address!=NULL)
	{
		if(destructor!=NULL)
		{
			...
			//	get element count
			element_count = *(unsigned char *)((unsigned char *)array_address-sizeof(_CXXABI_SIZE_T));
