...
size_t
HSST_read ( void* ptr, size_t size, size_t nmemb, HSST_STREAM* rs )
{
	size_t to_read, read;
	...
	read_header_ptr[_HSST_PACKET_CMD_FIELD] = HSST_READ;
	read_header_ptr[_HSST_PACKET_DES_FIELD] = rs->_stream;
	read_header_ptr[_HSST_PACKET_SIZE_FIELD] = size;
	read_header_ptr[_HSST_PACKET_NMEMB_FIELD] = nmemb;
	...
	if ( sizeof(read_header) == _HSST_recv ((void *)&read_header, sizeof(read_header) ) )
	{
		...
		if ( ( read_header_ptr[_HSST_PACKET_DES_FIELD] != rs->_stream ) ||
